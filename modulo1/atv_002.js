...
if (tentativaSenha === senhaFixa) {
    ...
    do {
        ...
        if (tentativas >= 2) console.log("[TENTATIVAS ESGOTADAS]");
...
/* let arrayAlunos = [["Arlindo", 8], ["Bruno", 6], ["Carlos", 3], ["Daniel", 9], ["Elias", 8]];
let maiorNota = arrayAlunos[0];
let menorNota = arrayAlunos[0];
let mediaTurma = 0;

for (const a of arrayAlunos){
    if (a[1] > maiorNota[1]) {
        maiorNota = a;
    }
    if (a[1] < menorNota[1]) {
...
for (const p of carrinhoCompras) {
    console.log(`- ${p[0]} - R$${p[1]}`);
}
console.log(`----------`);
